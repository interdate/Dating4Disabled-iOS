...
var app = {
	...
	UIHandler: function(){
		...
		if(app.logged === false){
			...
			app.currentPageWrapper = $('#'+app.currentPageId);
			$('#upload_image_page .regInfo').text('');
		}
	},
	...
	pushNotificationInit: function(){
		...
		push.on('notification', function(data) {
			...
			console.log("PUSH PLUGIN SUCCESS: " + JSON.stringify(data));
	...
	getSettings: function(name, value){
		...
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/settings',
			type: 'Get',
	...
	changeSettings: function(name, value){
		...
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/settings/' + name + '/' + value,
			type: 'Post',
			error: function(response){
				console.log(JSON.stringify(response));
			},
...
//				console.log(app.response.error);
	...
	addToFavorites: function(userId){
		...
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/favorites/' + userId,
			type: 'Post',
			...
			success: function(response, status, xhr){
				if(response.success){
					app.container.find('.addToFav').hide();
					app.container.find('.delFromFav').show();
					app.alert('The user has been added to your favorites list');
				}
				else{
					app.alert('The user already exists in your favorites list');
				}
			}
		});
	},
	...
	addToBlackList: function(userId){
		...
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/blacklist/' + userId,
			type: 'Post',
			contentType: "application/json; charset=utf-8",
			error: function(response){
				//alert(JSON.stringify(response));
			...
			success: function(response, status, xhr){
				if(response.success){
					app.container.find('.addToBlack').hide();
					app.container.find('.delFromBlack').show();
					app.alert('The user has been added to your black list');
				}
				else{
					app.alert('The user already exists in your black list');
				}
			}
		});
	},

	deleteFromFavorites: function(userId){
		
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/favorites/' + userId + '/delete',
			type: 'Post',
			contentType: "application/json; charset=utf-8",
			error: function(response){
				//alert(JSON.stringify(response));
			...
			success: function(response, status, xhr){
				console.log(JSON.stringify(response));
				if(response.success){
					app.container.find('.delFromFav').hide();
					app.container.find('.addToFav').show();
					app.alert('The user has been deleted from your favorites list');
				}
				else{
					app.alert('The user does not exist in your favorites list');
				}
			}
		});
	},

	deleteFromBlackList: function(userId){	
		
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/blacklist/' + userId + '/delete',
			type: 'Post',
			contentType: "application/json; charset=utf-8",
			error: function(response){
				//alert(JSON.stringify(response));
			...
			success: function(response, status, xhr){
				...
				if(response.success){
					app.container.find('.delFromBlack').hide();
					app.container.find('.addToBlack').show();
					app.alert('The user has been deleted from your black list');
				}
				else{
					app.alert('The user does not exist in your black list');
				}
			}
		});
	},
	...
	getUserProfile: function(userId){
		...
		$.ajax({
			...
			success: function(user, status, xhr){
				...
				if(user.lookingForOptions.length > 0){
					html = html + app.getProfileLine("I am looking for", user.lookingForOptions);
				}
				...
				html = html + app.getProfileLine("Zodiac Sign", user.zodiac);
				...
				if(user.isFavorite){
					detailsContainer.find('.addToFav').hide();
					detailsContainer.find('.delFromFav').show();
				}
				else{
					detailsContainer.find('.delFromFav').hide();
					detailsContainer.find('.addToFav').show();
				}


				if(user.isBlackListed){
					detailsContainer.find('.addToBlack').hide();
					detailsContainer.find('.delFromBlack').show();
				}
				else{
					detailsContainer.find('.delFromBlack').hide();
					detailsContainer.find('.addToBlack').show();
				}

				console.log(detailsContainer.html());
		...
		if(app.currentPageId != 'login_page' && app.currentPageId != 'register_page' && app.currentPageId != 'forgot_password_page'){
			checkNewMessagesRequest = $.ajax({
				...
				success: function(response){
                    ...
                    if(app.response.newNotificationsCount > 0){
                        app.newNotificationsCount = app.response.newNotificationsCount;
...
//		entityContainer['interests'] = '.interestsList';
	...
	getUsersForLikes: function(supposedToBeLikedUserId, notifId){
		...
		var url = 'http://m.dating4disabled.com/api/v6/users/forLikes/' + supposedToBeLikedUserId + '/' + notifId;
		//alert(url);
		//return;
	...
	doLike: function(){
		...
		$.ajax({
			url: 'http://m.dating4disabled.com/api/v6/user/like/' + userId,
			type: 'Post',
			error: function(error){
    ...
    getLikesNotifications: function(){
        ...
        $.ajax({
            url: 'http://m.dating4disabled.com/api/v6/user/likes/notifications',
            type: 'Get',
            error: function(error){
                //console.log("ERROR: " + JSON.stringify(error));
            },
    ...
    checkBingo: function(){
        ...
        if(app.currentPageId != 'login_page' && app.currentPageId != 'register_page' && app.currentPageId != 'recovery_page'){
            ...
            checkBingo = $.ajax({
                url: 'http://m.dating4disabled.com/api/v6/user/bingo',
                type: 'Get',
                error: function(error){
                    console.log("ERROR: " + JSON.stringify(error));
                },
    ...
    setBingoAsSplashed: function(bingo, i){
        ...
        $.ajax({
            url: 'http://m.dating4disabled.com/api/v6/user/bingo/splashed',
            type: 'Post',
            data: data,
            error: function(error){
                console.log("ERROR: " + JSON.stringify(error));
    ...
    setUserNotificationAsRead: function(notifId, clickedObj){
        $.ajax({
            url: 'http://m.dating4disabled.com/api/v6/user/notification/' + notifId + '/read',
            type: 'Post',
            error: function(error){
                console.log("ERROR: " + JSON.stringify(error));
            },
